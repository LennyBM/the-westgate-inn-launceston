...
import { USPStrip, OpeningHours, MapEmbed } from '../components/InfoBlocks';
            ...
            <div className="relative">
              ...
              {site.established && (
                <div className="absolute -bottom-5 -right-3 md:-right-5 bg-primary text-contrast rounded-xl px-5 py-3 shadow-lg">
                  <span className="font-headline italic text-2xl">Est. {site.established}</span>
                </div>
              )}
            </div>
          </Reveal>
            ...
            {offerCards().map((c, i) => (
              <Reveal key={c.title}>
                <Link to={c.to} className="card group block h-full" style={{ transitionDelay: `${i * 80}ms` }}>
                  ...
                  <div className="p-6">
                    <h3 className="font-headline italic text-2xl text-ink">{c.title}</h3>
      ...
      <section className="section bg-surface">
        <div className="container-x grid lg:grid-cols-2 gap-12 items-center">
          ...
          <Reveal>
            ...
            <hr className="rule mt-5" />
            <address className="not-italic mt-5 text-ink/80 leading-relaxed">
